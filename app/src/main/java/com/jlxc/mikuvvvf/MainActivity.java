...
public class MainActivity extends Activity {
    ...
    private final Runnable autoRunnable = new Runnable() {
        @Override public void run() {
            if (!autoTest) return;
            autoSpeed += autoDirection * 0.10f; // V7：按 0→140km/h 样本节奏慢慢爬升，方便听真实换段。
    ...
    private View buildContentView() {
        ...
        title.setText("Miku VVVF / Engine Sound Demo V8");
        ...
        sub.setText("车速绑定声浪模拟 V8 · MainApp Hook 数据源 + 500ms 平滑 · UDP 47230");
        sub.setTextSize(14);
        ...
        TextView hookHint = new TextView(this);
        hookHint.setText("Hook 开启时，手动滑条/UDP SPEED 只作为离车调试；实车上会被 CarInfoService 的车速覆盖。声音内部会用 0.72s 时间常数平滑追随，避免 500ms 数据源导致一卡一卡。");
        hookHint.setTextSize(12);
        hookHint.setTextColor(Color.DKGRAY);
        hookHint.setPadding(0, dp(2), 0, dp(8));
        root.addView(hookHint, matchWrap());

        Button auto = new Button(this);
        auto.setText("真实采样自动 0→140→0 测试（离车调试请先关 Hook）");
        ...
        stageHint.setText("V8：默认直接读取 MainApp Hook 数据源。CarInfoService 只读 requestCarBaseInfo()，不调用高风险车门接口；轮询最低 500ms。音频线程会对车速/RPM 做连续插值，避免低频数据导致采样 VVVF 发抖或发动机声一卡一卡。");
    ...
    private void updateInfoText() {
        ...
        String text = "局域网 UDP 指令：\n"
                ...
                + "  echo HOOK 0 | nc -u " + host + " 47230    # 关闭 Hook，方便手动/UDP 测试\n"
                + "  echo HOOK 1 | nc -u " + host + " 47230    # 开启 MainApp Hook 车速\n"
                + "  echo POLL 500 | nc -u " + host + " 47230   # 设置 Hook 轮询，低于 500 会自动钳制\n\n"
                + "ADB 调试：\n"
                ...
                + "实车默认不需要 MikuCarLauncher 再发 SPEED；本 App 会直接绑定 com.ts.MainUI 的 CarInfoService 读取车速/RPM。UDP/ADB 保留给离车调试。";
